  ...
  try {
    ...
    if(callResult.response!=undefined){
      ...
      callResult.data.data.map((value, index) => {          
       ...
       yield put({
         type: actions.GET_WORKORDER_STATUS_REDUCER,
         status:temp,//createDemoData(),
       });
    }  
...
export function* getById({payload}) {
  ...
  try {
    const callResult = yield call(
      onCallReqeust,
      `${siteConfig.apiUrl}/workorderstatus/${payload.id}`             
    );
...
export function* addWorkOrderStatus({payload}) { 
    ...
    if(callResult.response!=undefined){
      notification('error',callResult.response.data.msg)     
       ...
       // notification('success',callResult.data.msg)
        yield put({
          type: actions.GET_WORKORDER_STATUS,
...
export function* deleteAssetCategory({payload}) { 
  ...
  try {
    const callResult = yield call(
      onDeleteCallReqeust,     
      `${siteConfig.apiUrl}/assetcategory/${payload.id}`          
    );
    if(callResult.response!=undefined){
      notification('error',callResult.response.data.msg)     
    }
...
export default function* rootSaga() {
  yield all([
    yield takeEvery(actions.GET_WORKORDER_STATUS,   getOrderWorkStatus),
    yield takeEvery(actions.ADD_WORKORDER_STATUS, addWorkOrderStatus),
