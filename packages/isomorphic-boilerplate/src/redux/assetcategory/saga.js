...
export function* getAssetCategories() {
  ...
  try {
    const callResult = yield call(
      onCallReqeust,
      `${siteConfig.apiUrl}/assetcategory`             
    );
    ...
    if(callResult.response!=undefined){
      ...
      callResult.data.data.map((value, index) => {          
           ...
           value.parentid=value.intParentID
           temp.push(value);
       });     
      
       yield put({
         type: actions.GET_ASSET_CATEGORY_REDUCER,
         assetcategories:temp,//createDemoData(),
       });
    }  
...
export function* addAssetCategory({payload}) { 
...
export default function* rootSaga() {
  yield all([
    yield takeEvery(actions.GET_ASSET_CATEGORIES,   getAssetCategories),
    yield takeEvery(actions.ADD_ASSET_CATEGORY, addAssetCategory),
