...
export function* getDrillCategories() {
  ...
  try {
    const callResult = yield call(
      onCallReqeust,
      `${siteConfig.apiUrl}/drillcategory`             
    );
    ...
    if(callResult.response!=undefined){
      ...
      callResult.data.data.map((value, index) => {          
       ...
       yield put({
         type: actions.GET_DRILL_CATEGORIES_REDUCER,
...
export function* getById({payload}) {
  ...
  try {
    const callResult = yield call(
      onCallReqeust,
      `${siteConfig.apiUrl}/drillcategory/${payload.id}`             
    );
...
export function* addDrillCategory({payload}) { 
  ...
  try {
    ...
    if(callResult.response!=undefined){
      notification('error',callResult.response.data.msg)     
        ...
        yield put({
          type: actions.GET_DRILL_CATEGORIES,          
        });
    }
  }
  catch (error) {  
...
export function* updateDrillCategory({payload}) { 
...
export function* deleteDrillCategory({payload}) { 
...
export default function* rootSaga() {
  yield all([
    yield takeEvery(actions.GET_DRILL_CATEGORIES,   getDrillCategories),
    yield takeEvery(actions.ADD_DRILL_CATEGORY, addDrillCategory),
    // yield takeEvery(actions.GET_BY_ID, getById),
    yield takeEvery(actions.UPDATE_DRILL_CATEGORY, updateDrillCategory),
    yield takeEvery(actions.DELETE_DRILL_CATEGORY, deleteDrillCategory), 
