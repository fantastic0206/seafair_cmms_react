...
const {  getCalendarData, updateCalendarWorkOrderData ,updateCalendarCharterData} = CalendarActions;
...
export default function DndCalendar() {
  ...
  const selectedFilter = (row) => { // filter by user and group
    ...
    console.log(filterVal2.intSysCode,row)
    var temp = [];
    if (filterVal2.intSysCode == "open" && Object.keys(row).length == 0) {
      ...
      return;
    } else if (  filterVal2.intSysCode == "closed" && Object.keys(row).length == 0 ) {
      events.forEach((item) => {
        if(item.type=="workorder"){
          if (item.intWorkOrderStatusID == 7 || item.intWorkOrderStatusID == 9) {
            temp.push(item);
      ...
      setFilterTxt("(Built in Filter) All Work Orders");
      return;
    } else if ((filterVal2.intSysCode != "closed" && Object.keys(row).length == 0) || (filterVal2.intSysCode != "open" && Object.keys(row).length == 0)) {
      events.forEach((item) => {
        if(item.type=="workorder"){
      ...
      if (row._id == localStorage.getItem("user_id")) {
        ...
        events.forEach((item) => {
          var rowId = item.intAssignedUserId != null ? item.intAssignedUserId._id : "";
            if(item.type=="workorder"){
                if (
                  filterVal2.intSysCode == "open" || filterVal2.intSysCode == undefined ) {
                  if (
                    rowId == row._id &&
  ...
  const selectStatusFilter = (row) => { // filter by work order status
    ...
    if (row.intSysCode == "open" && Object.keys(filterVal).length == 0) {
      ...
      return;
    } else if ( (row.intSysCode != "closed" && Object.keys(filterVal).length == 0) || (row.intSysCode != "open" && Object.keys(filterVal).length == 0) ) {
