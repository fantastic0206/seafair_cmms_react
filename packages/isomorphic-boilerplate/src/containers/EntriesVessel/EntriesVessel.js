...
export default function Audits() {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: "25%",  
      render: (text,row) => {      
        return <a onClick={()=>goDetail(row._id)}>{moment(text).format("YYYY-MM-DD HH:mm:ss")}</a>}
      },
    {
      title: "Date Revised",
      ...
      render: (text,row) => {
        ...
        return <a onClick={()=>goDetail(row._id)}>{text!=null?moment(text).format("YYYY-MM-DD HH:mm:ss"):""}</a>}
      },    
    {
      title: "Date printed",
      ...
      render: (text,row) => {   
        ...
        return <a onClick={()=>goDetail(row._id)}>{printedDate!=null?moment(printedDate).format("YYYY-MM-DD HH:mm:ss"):""}</a>}      
      },
    {
      title: "Status ",
