...
const { updateData ,add, deleteData, getById} = scheduledDrillAction;
...
export default function (props) {
  ...
  const { id } = useParams();
  const { scheduleddrill, isDelete } = useSelector((state) => state.ScheduledDrill);
  ...
  const [strAssigneeUser,setStrAssigneeUser]=React.useState("");
  ...
  React.useEffect(() => {
    if(Object.keys(scheduleddrill).length !== 0){
      // console.log(scheduleddrill);
      if(scheduleddrill._id != id)
        return;
      // console.log(audit);
      setStrDrillTitle(scheduleddrill.strTitle);
      setDrillTypeName(scheduleddrill.strType);
      setBolScheduledDrillStatus(scheduleddrill.bStatus);
      setDrillCategoryName(scheduleddrill.strCategory);
      setIntEstimatedTime(scheduleddrill.intEstimatedTime);
      setDueDate(scheduleddrill.aDueDate);
      setStrAssigneeUser(scheduleddrill.strGeneralAssignee);
      setStrDrillDescription(scheduleddrill.strGeneralDescription);
  ...
  return (
    <LayoutWrapper>
      <div className="PageHeader">
        ...
        <Button type="primary"  className="updateBtn" onClick={onUpdate} style={margin}>
          <span>Update</span>
        </Button>
        <Button type="danger"  className="deleteBtn" onClick={onDelete} style={margin}>
          <span>Delete</span>
        </Button>
      </div>
      <TableTabsStyle className="isoLayoutContentAsset">
        ...
        >
          <TabPane tab="General" key="1">
            <General
              pageState={"add"}
              data = { {strAssigneeUser, strDrillDescription} }
