...
const { getById, updateData, deleteData } = scheduledAuditAction;
...
export default function (props) {
  ...
  const { id } = useParams();
  const { scheduledaudit, isDelete } = useSelector((state) => state.ScheduledAudit);
  ...
  React.useEffect(() => {
    if(Object.keys(scheduledaudit).length !== 0){
      console.log(scheduledaudit);
      if(scheduledaudit.scheduledaudit._id != id)
        return;
      // console.log(scheduledaudit);
      setStrTitle(scheduledaudit.scheduledaudit.strTitle);
      setStrStatus(scheduledaudit.scheduledaudit.strStatus);
      setStrCode(scheduledaudit.scheduledaudit.strCode);
      setStrVessel(scheduledaudit.scheduledaudit.strDepartmentVessel);
      setStrAuditor(scheduledaudit.scheduledaudit.strAuditor);
      setStrAuditee(scheduledaudit.scheduledaudit.strAuditee);
      setStrISMReference(scheduledaudit.scheduledaudit.strISMReference);
      setStrSMSReference(scheduledaudit.scheduledaudit.strSMSReference);
      setBolScheduledAuditStatus(scheduledaudit.scheduledaudit.bolScheduledAuditStatus);
      setAPlan(scheduledaudit.scheduledaudit.aSmsAuditPlan);
      setAInspectionData(scheduledaudit.scheduledaudit.aSmsAuditInspection);
      setAReport(scheduledaudit.scheduledaudit.aSmsAuditReport);
      setANCRCAR(scheduledaudit.scheduledaudit.aNCRCAR);
    }
  }, [scheduledaudit])
